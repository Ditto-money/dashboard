...
export default function ({ id, notification }) {
  ...
  return (
    <Paper className={clsx(classes.paper, classes[notification.type])}>
      ...
      <div className={clsx('flex', 'flex-grow', 'items-center', classes.container)}>
        <div className={clsx('flex', 'flex-grow', 'flex-col', 'justify-center')}>
...
function TxContent({ notification }) {
  ...
  return (
    <>
      ...
      <a
        href={`https://${IS_TESTNET ? 'testnet.' : ''}bscscan.com/tx/${notification.hash}`}
        target="_blank"
...
function ErrorContent({ notification }) {
  ...
  return (
    <>
      <strong className={clsx(classes.small, classes.error)}>{notification.message}</strong>
    </>
  )
}
...
function SuccessContent({ notification }) {
  ...
  return (
    <>
      ...
      <strong className={clsx(classes.small, classes.success)}>{notification.message}</strong>
    </>
  )
}
