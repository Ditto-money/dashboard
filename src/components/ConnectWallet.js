...
export default function () {
  ...
  return (
    <Dialog onClose={() => {}} aria-labelledby="wrong-network-prompt" open={wallet.isOnWrongNetwork || wallet.isConnecting}>
      <div className={clsx('flex', 'flex-grow', 'flex-col', classes.container)}>
        {!wallet.isOnWrongNetwork ? (
          ...
          <Box mt={2} className={clsx('flex', 'flex-col', 'items-center', 'justify-center', 'text-center')}>
            <div>You are connected to the wrong network</div>
            <strong>
              Please connect to Binance Smart Chain <span className={classes.net}>{NETWORK_NAME}</span>
            </strong>
            <div>or</div>
